...
def parse_keep_jobs(keep_jobs):
    ...
    for job in keep_jobs:
        ...
        if search:
            for entry in search:
                if entry[1] == "y":
                    replacement = int(entry[0]) * 31536000
                elif entry[1] == "M":
                    replacement = int(entry[0]) * 2628000
                elif entry[1] == "d":
                    replacement = int(entry[0]) * 86400
                elif entry[1] == "h":
                    replacement = int(entry[0]) * 3600
                elif entry[1] == "m":
                    replacement = int(entry[0]) * 60
                else:
...
def log(message, target="ERROR"):
    global LOGFILTER
    if target.upper().startswith("V"):
        target = 1
    elif target.upper().startswith("D"):
        target = 2
    else:
        target = 0
    fmt = "%Y-%m-%d %H:%M:%S"
...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "hf:k:sv",
                                   ["help", "filter=", "exclude=", "keep=", "create-symlinks", "verbose", "run",
                                    "debug", "zfs-binary="])
